...
<div class="flex items-center justify-between mb-4">
    ...
    @permission('internal_inventory_categories.create')
...
<div class="mb-4">
    <form action="{{ route('internal.inventory.index') }}" method="GET" class="flex items-center gap-2">
        ...
        <button type="button" onclick="window.location.href='{{ route('internal.inventory.index', ['tab' => 'categories']) }}'" class="inline-flex items-center gap-2 px-3 text-white text-xs font-medium rounded transition" style="min-height: 32px; background-color: #dc2626;">
...
>
    ...
    <tr class="hover:bg-gray-50 {{ !$category->is_active ? 'opacity-50' : '' }}">
        ...
        <td class="px-6 py-4 whitespace-nowrap text-center text-sm font-medium">
            <x-ui.action-buttons
                :edit-onclick="auth()->user()->hasPermission('internal_inventory_categories.update') ? 'editCategory(' . $category->id . ', ' . json_encode(['name' => $category->name, 'description' => $category->description]) . ')' : null"
                :delete-onclick="auth()->user()->hasPermission('internal_inventory_categories.delete') ? 'deleteCategory(' . $category->id . ')' : null"
                :more-actions="auth()->user()->hasPermission('internal_inventory_categories.update') ? [
...
<!-- Category Modal -->
<div id="categoryModal" style="display: none; position: fixed; top: 0; left: 0; right: 0; bottom: 0; background-color: rgba(0,0,0,0.5) !important; z-index: 9999 !important;">
    <div style="position: fixed; top: 50%; left: 50%; transform: translate(-50%, -50%); background-color: #ffffff !important; border-radius: 12px !important; box-shadow: 0 25px 50px -12px rgba(0,0,0,0.25) !important; width: 100% !important; max-width: 480px !important; overflow: hidden !important; z-index: 10000;">
        <div style="padding: 16px 20px !important; border-bottom: 1px solid #e5e7eb !important; display: flex !important; align-items: center !important; justify-content: space-between !important; background-color: #f9fafb !important;">
            <div style="display: flex !important; align-items: center !important; gap: 10px !important;">
                ...
                <h3 id="categoryModalTitle" style="font-size: 14px !important; font-weight: 600 !important; color: #111827 !important; font-family: Poppins, sans-serif !important; margin: 0 !important;">Add Category</h3>
            </div>
            <button type="button" onclick="closeCategoryModal()" style="width: 32px !important; height: 32px !important; border-radius: 6px !important; border: none !important; background-color: transparent !important; cursor: pointer !important; display: flex !important; align-items: center !important; justify-content: center !important;" onmouseover="this.style.backgroundColor='#e5e7eb'" onmouseout="this.style.backgroundColor='transparent'">
...
function openCategoryModal() {
    ...
    document.getElementById('categoryForm').action = '{{ route("internal.inventory.categories.store") }}';
...
function editCategory(id, data) {
    ...
    document.getElementById('categoryForm').action = '/internal/inventory/categories/' + id;
...
function deleteCategory(id) {
    window.showDeleteModal('/internal/inventory/categories/' + id);
}
