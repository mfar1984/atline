@php
    $credentials = $paymentSetting->getDecryptedCredentials();
    $isDisabled = !auth()->user()->hasPermission('settings_integrations_payment.update');
@endphp
        ...
        <div class="flex items-center justify-between">
            <div class="flex items-center gap-3">
                <div class="w-10 h-10 rounded-full flex items-center justify-center" style="background-color: #fef3c7;">
                    <span class="material-symbols-outlined" style="font-size: 20px; color: #d97706;">payments</span>
                </div>
                <div>
                    <h3 class="text-sm font-semibold text-gray-900" style="font-family: Poppins, sans-serif;">Payment Gateway</h3>
                    <p class="text-xs text-gray-500">Chip-Asia Payment Integration</p>
                </div>
            </div>
            <span class="inline-flex items-center px-2 py-1 rounded text-xs font-medium {{ $paymentSetting->isConnected() ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-600' }}">
                {{ $paymentSetting->getStatusLabel() }}
            </span>
        </div>
        @if($paymentSetting->last_tested_at)
            <p class="text-xs text-gray-400 mt-2">Last tested: {{ $paymentSetting->last_tested_at->format('d/m/Y H:i') }}</p>
        @endif
    </div>

    <!-- Info Box -->
    <div class="p-4 rounded-lg" style="background-color: #eff6ff; border: 1px solid #bfdbfe;">
        <div class="flex items-start gap-2">
            ...
            <div class="text-xs" style="color: #1e40af;">
                <p class="font-medium">Chip-Asia Payment Gateway</p>
                <p class="mt-1">Get your Brand ID and Secret Key from <a href="https://gate.chip-in.asia" target="_blank" class="underline">Chip-Asia Dashboard</a></p>
            </div>
        </div>
    </div>

    <!-- Configuration Form -->
    <form method="POST" action="{{ route('settings.integrations.payment.update') }}">
        @csrf
        
        <div class="mb-4">
            <label class="block text-gray-700 mb-1" style="font-size: 11px;">Brand ID <span class="text-red-500">*</span></label>
            <input type="text" name="brand_id" value="{{ $credentials['brand_id'] ?? '' }}" 
                   ...
                   style="min-height: 32px; font-size: 11px;" placeholder="Enter your Brand ID" {{ $isDisabled ? 'disabled' : '' }}>
        </div>

        <div class="mb-4">
            <label class="block text-gray-700 mb-1" style="font-size: 11px;">Secret Key</label>
            <input type="password" name="secret_key" 
                   class="w-full px-3 border border-gray-300 rounded text-gray-700 focus:ring-1 focus:ring-blue-500 focus:border-blue-500 {{ $isDisabled ? 'bg-gray-100' : '' }}"
                   style="min-height: 32px; font-size: 11px;" placeholder="{{ isset($credentials['secret_key']) ? '••••••••' : 'Enter your Secret Key' }}" {{ $isDisabled ? 'disabled' : '' }}>
        </div>

        <div class="mb-4">
            <label class="block text-gray-700 mb-1" style="font-size: 11px;">Webhook URL (Optional)</label>
            <input type="url" name="webhook_url" value="{{ $credentials['webhook_url'] ?? '' }}" 
                   class="w-full px-3 border border-gray-300 rounded text-gray-700 focus:ring-1 focus:ring-blue-500 focus:border-blue-500 {{ $isDisabled ? 'bg-gray-100' : '' }}"
                   style="min-height: 32px; font-size: 11px;" placeholder="https://your-domain.com/payment/webhook" {{ $isDisabled ? 'disabled' : '' }}>
            <p class="text-xs text-gray-400 mt-1">URL to receive payment notifications</p>
        </div>

        <!-- Action Buttons -->
        <div class="flex items-center border-t border-gray-200" style="gap: 8px; padding-top: 20px; margin-top: 16px;">
            @permission('settings_integrations_payment.update')
            <button type="submit" 
                    class="inline-flex items-center gap-2 px-4 text-white rounded hover:opacity-90 transition"
                    style="min-height: 32px; font-size: 11px; font-family: Poppins, sans-serif; background-color: #d97706;">
                <span class="material-symbols-outlined" style="font-size: 14px;">save</span>
                Save Settings
            </button>
            @endpermission
            <button type="button" onclick="testPaymentConnection()"
...
function testPaymentConnection() {
    ...
    fetch('{{ route("settings.integrations.payment.test") }}', {
        method: 'POST',
        headers: {
